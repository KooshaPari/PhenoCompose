export interface PhenoDocsLinkItem {
  text: string;
  link: string;
}

export interface PhenoDocsNavGroup {
  text: string;
  items: readonly PhenoDocsLinkItem[];
}

export type PhenoDocsNavItem = PhenoDocsLinkItem | PhenoDocsNavGroup;

export interface PhenoDocsSidebarSection {
  text: string;
  items: readonly PhenoDocsLinkItem[];
}

export interface PhenoDocsSearchTranslations {
  button: {
    buttonText: string;
...
export interface PhenoDocsSearchConfig {
  provider: 'local';
  options: {
    detailedView: boolean;
    translations: PhenoDocsSearchTranslations;
    miniSearch: {
      options: {
...
export interface PhenoDocsThemeConfig {
  nav: readonly PhenoDocsNavItem[];
  sidebar: Record<string, readonly PhenoDocsSidebarSection[]>;
  search: PhenoDocsSearchConfig;
  socialLinks: readonly Array<{
    icon: string;
    link: string;
  }>;
}

export interface PhenoDocsSiteConfig {
  title: string;
  description: string;
  themeConfig: PhenoDocsThemeConfig;
}

export const phenoDocsConfig: PhenoDocsSiteConfig = {
  title: 'NanoVMS',
  ...
  themeConfig: {
    ...
    socialLinks: [{ icon: 'github', link: 'https://github.com/KooshaPari/nanovms' }]
  }
};
